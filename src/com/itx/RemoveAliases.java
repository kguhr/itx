...
public class RemoveAliases implements IProgramBase
{
	public static void main(String[] args)
	{
		...
		} else
		{
			...
			try
			{
				...
				new RemoveAliases().run(enterpriseSession, infoStore, new String[] {args[3], args[4]});
	...
	public void run(IEnterpriseSession enterpriseSession, IInfoStore infoStore, String[] args) throws SDKException
	{
		...
		} else
		{
			...
			for (Iterator iterator = objects.iterator(); iterator.hasNext();)
			{
				...
				if(hasEnterprise)
				{
					System.out.println("\tRemoving Alias: secEnterprise");
					object.getAliases().remove("secEnterprise:" + object.getTitle());
